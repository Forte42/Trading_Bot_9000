def get_binance_btc_3m_data():

	""" Gets trade by trade data from Binance and writes it to 'binance_btc_3m' table in MySQL crypto DB """
	...
	ohlc_list = (client.klines("BTCUSDT", "3m", limit = 1000))
	...
	try:
		last_timestamp = engine.execute("SELECT Timestamp FROM binance_btc_3m ORDER BY timestamp DESC LIMIT 1;")
		last_timestamp = last_timestamp.fetchone()
		last_timestamp = last_timestamp[0]
		print('Successfully acquired last timestamp from binance_btc_3m')
	...
	if last_timestamp < 99999999999999999999:
		...
		df.to_sql(con=engine, name='binance_btc_3m', if_exists='append',chunksize=100, index=False)
