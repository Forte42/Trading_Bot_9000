...
while True:
	...
	df = pd.read_sql(f"SELECT * FROM binance_btc_trades ORDER BY id DESC LIMIT 600", conn)   
	df.index = df.set_index(pd.DatetimeIndex(df["id"]))

	df['price'] = df['price'].astype(float)                                                                                
	df['qty'] = df['qty'].astype(float)
	df['lastprice'] = df['price'].shift(1).astype(float)
	                                                                       
	df['vprice'] = (df['price']-df['lastprice'])*df['qty']
	df['price'] = df['vprice'].rolling(2).mean()/df['qty'].rolling(2).mean()
	df['lastprice'] = df['price'].shift(1).astype(float)
	...
	df["EMA100"] = ta.ema(df.price, length=100)
	df["EMA150"] = ta.ema(df.price, length=150)
