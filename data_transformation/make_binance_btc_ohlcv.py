...
def return_dataframe(period):
	...
	df = pd.read_sql(f"SELECT * FROM binance_btc_{period}", conn)
	...
	df['Bar_Change_30MA'] = df['Bar_Change'].rolling(30).mean()
	df['Bar_Change_10MA'] = df['Bar_Change'].rolling(10).mean()
	df['Bar_Change_5MA'] = df['Bar_Change'].rolling(5).mean()
	...
	df['Consecutive'] = df['Updown'].groupby((df['Updown'] != df['Updown'].shift()).cumsum()).cumcount() + 1
